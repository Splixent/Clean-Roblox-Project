...
local function CreateSlotItem(slotScope, slotData, slotIndex: number, coolingTableLevel: number?)
    ...
    -- Determine if drying or cooling (drying = has dryingStartTime and not yet dried)
    local dryingElapsed = slotData.dryingStartTime and (os.time() - slotData.dryingStartTime) or 0
    local isDrying = slotData.dryingStartTime ~= nil and not slotData.dried and dryingElapsed < dryingDuration
    ...
    local progressColor = isDrying and dryingColor or coolingColor
    
    -- Calculate correct start time, end time, and remaining time based on drying vs cooling
    local totalDuration
    if isDrying then
        totalDuration = dryingDuration
    else
        totalDuration = coolingDuration
    end
    ...
    local updateConnection = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Recalculate drying elapsed to determine current phase (using tick for precision)
        local currentDryingElapsed = slotData.dryingStartTime and (currentTick - (slotData.dryingStartTime + timeOffset)) or 0
        local nowDrying = slotData.dryingStartTime ~= nil and not slotData.dried and currentDryingElapsed < dryingDuration
        
        -- Calculate remaining time based on current phase (sub-second precision)
        local remaining
        if nowDrying then
            local dryingEndTick = slotData.dryingStartTime + timeOffset + dryingDuration
            remaining = math.max(0, dryingEndTick - currentTick)
        else
            local endTick = slotData.endTime + timeOffset
            remaining = math.max(0, endTick - currentTick)
        end
        
        RemainingTime:set(remaining)
        
        -- Flash green when complete
        local allComplete = remaining <= 0 and not nowDrying
        if allComplete then
        ...
        -- Update viewport model color based on drying progress
        if slotData.dryingStartTime and not slotData.dried then
            local dryingProgress = ScriptUtils:GetDryingProgress(slotData.dryingStartTime, dryingDuration)
            local currentColor = ScriptUtils:GetDryingColor(clayType, dryingProgress)
    ...
    if viewportModel then
        local currentColor = clayTypeData.color
        if slotData.dried then
            currentColor = clayTypeData.driedColor or clayTypeData.color
        elseif slotData.dryingStartTime then
            local dryingProgress = ScriptUtils:GetDryingProgress(slotData.dryingStartTime, dryingDuration)
            currentColor = ScriptUtils:GetDryingColor(clayType, dryingProgress)
        end
...
return s:New "Frame" {
    ...
    [Children] = {
        ...
        s:New "TextButton" {
            ...
            Rotation = Functions.ExitRotation,
            Size = s:Computed(function(use)
                local scale = use(Functions.ExitScale)
                return UDim2.fromScale(0.099 * scale, 0.236 * scale)
            end),
            Text = "X",
            TextColor3 = Color3.fromRGB(255, 0, 4),
            TextScaled = true,
            ...
            [OnEvent "MouseEnter"] = function()
                if UserInputService.PreferredInput == Enum.PreferredInput.KeyboardAndMouse then
                    Functions.ExitHovered:set(true)
                end
            end,
            
            [OnEvent "MouseLeave"] = function()
                Functions.ExitHovered:set(false)
                Functions.ExitPressed:set(false)
            end,
            
            [OnEvent "MouseButton1Down"] = function()
                Functions.ExitPressed:set(true)
            end,
