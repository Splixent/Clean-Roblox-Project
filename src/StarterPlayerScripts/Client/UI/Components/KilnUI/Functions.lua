...
Functions.MaxSlots = scope:Value(2)
Functions.KilnLevel = scope:Value(0)
...
-- Show the kiln UI with data
function Functions:Show(slotsData: {[string]: any}, maxSlots: number, kilnLevel: number?, onCollect: ((number) -> ())?, onDelete: ((number) -> ())?, onClose: (() -> ())?)
    ...
    Functions.KilnLevel:set(kilnLevel or 0)
