...
local UIPostProcess = {
    ...
    scrollFrameJanitor = Janitor.new(),
    masterUI = nil,
    ...
    handlers = {
        ...
        },
        {
            ...
            action = function(object, context)
                ...
                if object.StrokeSizingMode == Enum.StrokeSizingMode.ScaledSize then return end
                if object:GetAttribute("original") == nil then
...
function UIPostProcess:UpdateAutoScaleScrollingFrames(masterUI)
    UIPostProcess.scrollFrameJanitor:Destroy()
    UIPostProcess.scrollFrameJanitor = Janitor.new()
    ...
    for index, scrollingFrame in ipairs(UIPostProcess.masterUI:GetDescendants()) do
        if scrollingFrame:IsA("ScrollingFrame") then
            ...
            local UIConstraint = scrollingFrame:FindFirstChildOfClass("UIListLayout") or scrollingFrame:FindFirstChildOfClass("UIGridLayout")
            if UIConstraint then
                table.insert(UIPostProcess.scrollingFrameConnections, UIPostProcess.scrollFrameJanitor:Add(UIConstraint:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
